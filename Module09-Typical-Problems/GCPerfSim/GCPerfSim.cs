...
/* Konrad's notes:
 * container: memory_mb: 600 in bench file is used to limit process with cgroups directly on Linux, not docker:
 * - ExecArgs(("cgcreate", "-g", _get_cgroup_categories_and_name(container)))
 * - ExecArgs(("cgset", "-r", f"memory.limit_in_bytes={mb_to_bytes(memory_mb)}", _CGROUP_NAME))
 * - cgexec ...
 * And job object on Windows
 */
...
/*
 * Used - potentially homework, not sure yet. Some problem with getting interesting data out of it...
 */
        ...
        while (true)
        {
            ...
            switch (word[0])
            {
                ...
                case 'p':
                    ...
                    break;
                case 'f':
                    text.Assert(word == "finalizableInterval");
                    finalizableInterval = text.TakeUInt();
                    break;
                case 'i':
                    text.Assert(word == "isPoh");
